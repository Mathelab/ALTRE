...
1. *writeConsensusRE()*: writes a CSV file in the working direcoty with the 
...
```{r, eval = FALSE}
writeConsensusRE(consensusPeaksAnnotated, "altreconspeaks.txt")
```

2. *writeBedFile()*: writes a BED file in the working directory with the location of REs, color-coded by type-specificity.  The output BED file can be directly loaded in a genome browser. There is no output to the function that can be saved as an object.

```{r, eval = FALSE}
writeBedFile(alteredPeaksCategorized, "altrecategpeaks.csv")
```
...
```{r, eval = FALSE}
writePathEnrich(MFenrich, "MFenrich.csv")
```

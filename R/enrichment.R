...
pathenrich <- function(analysisresults, ontoltype = "MF", enrichpvalfilt = 0.01, lfctypespecific = 1.5, lfcshared = 1.2, pvaltypespecific = 0.01, pvalshared = 0.05,
    genes = 20, offspring = 300, regionsubset = "promoter") {
    ...
    up = newanalysisresults[!(is.na(newanalysisresults$padj)) & newanalysisresults$log2FoldChange > lfctypespecific & newanalysisresults$padj < pvaltypespecific,
        ]
    down = newanalysisresults[!(is.na(newanalysisresults$padj)) & newanalysisresults$log2FoldChange < -lfctypespecific & newanalysisresults$padj <
        pvaltypespecific, ]
    shared = newanalysisresults[(newanalysisresults$log2FoldChange <= lfcshared & newanalysisresults$log2FoldChange >= -lfcshared) & (newanalysisresults$padj >=
        pvalshared | is.na(newanalysisresults$padj)), ]
    ...
    if (nrow(subsets[["up"]]) == 0) {
        ...
        expt = rundose(set = subsets[["up"]], background = subsets[["all"]], log2FoldChange = lfctypespecific, ontoltype = ontoltype, pvalfilt = enrichpvalfilt,
            genes = genes, offspring = offspring)
        if (nrow(shared) == 0) {
    ...
    if (nrow(subsets[["down"]]) == 0) {
        ...
        reference = rundose(set = subsets[["down"]], background = subsets[["all"]], log2FoldChange = lfctypespecific, ontoltype = ontoltype, pvalfilt = enrichpvalfilt,
            genes = genes, offspring = offspring)
        if (nrow(expt) == 0) {
    ...
    if (nrow(subsets[["shared"]]) == 0) {
        ...
        shared = rundose(set = subsets[["shared"]], background = subsets[["all"]], log2FoldChange = lfcshared, ontoltype = ontoltype, pvalfilt = enrichpvalfilt,
            genes = genes, offspring = offspring)

...
writeGREATpath <-
  function(pathenrichOut, con) {
    ...
    listNames <- names(pathenrichOut)
    pathNames <- names(pathenrichOut[[1]]$Sig_Pathways)
    fileNames=as.character(unlist(lapply(listNames,function(x) 
	paste(x,pathNames,sep="_"))))
    fileExt <- tools::file_ext(con)
   
    allfiles=c() 
    for(l in listNames) {
	for(p in pathNames) {
		fileCon <- stringr::str_replace(con, fileExt,
			stringr::str_c(paste(l,p,sep="_"),"csv",sep="."))

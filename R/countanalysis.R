...
#' merge=TRUE, regionspecific=TRUE,mergedistenh=1500,mergedistprom=1000 )
...
countanalysis <- function(counts, pval = 0.01, lfcvalue = 1) {
    ...
    keepers = intersect(which(resultsdataframe$padj < pval), which(abs(resultsdataframe$log2FoldChange) > lfcvalue))
    ...
    ####################### Get some stats
    tempstats = table(fulldataframe$meta.region[which(fulldataframe$log2FoldChange < 0)])
    ...
    stats = matrix(c(upproms, downproms, upenh, downenh), nrow = 2)
    colnames(stats) = c("Promoters", "Enhancers")
    stats = as.data.frame(stats)
    stats = cbind(c("UP", "DOWN"), stats)
    names(stats)[1] = "REtype"

    ####################### Produce a volcano plot and a barplot showing differential REs Find NAs to remove them, thereby avoiding printing warnings
    nonas = which(!is.na(resultsdataframe$padj))
    toplot = resultsdataframe[nonas, ]
    keepers = intersect(which(toplot$padj < pval), which(abs(toplot$log2FoldChange) > lfcvalue))
    toplot$col = rep("non-altered", nrow(toplot))
    toplot$col[keepers] = "altered"

    return(list(results = fulldataframe, stats = stats, dftoplot = list(toplot = toplot, pval = pval, lfcvalue = lfcvalue)))
}

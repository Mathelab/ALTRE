...
#' @param lfctypespecific log2fold change cutoff (of chromatin accessibility) for type specific enhancers/promoters
#' @param lfcshared log2fold change cutoff (of chromatin accessibility) for shared enhancers/promoters
#' @param pvaltypespecific p-value cutoff (of chromatin accessibility) for type specific enhancers/promoters
#' @param pvalshared p-value cutoff (of chromatin accessibility) for shared enhancers/promoters
#'
#' @examples
#' \dontrun{
...
#' counts_consPeaks=getcounts(annotpeaks=consPeaksAnnotated, sampleinfo=sampleinfo, reference='SAEC')
...
#'}
#' @return list 1) results of countanalysis, 2) Frequences of sample type specific, shared, and ambiguous regulatory elements 
#'
#' @export
 ...
 stop('analysisresults parameter is not in the correct format, make sure you are using the output from countanalysis()')
   ...
   stats=data.frame(REtype=c("Promoter","Enhancer"),
	Expt_Specific=c(length(intersect(which(analysisresults$region=="promoter"),
		which(analysisresults$REaltrecateg=="Experiment Specific"))),
		length(intersect(which(analysisresults$region=="enhancer"),
		which(analysisresults$REaltrecateg=="Experiment Specific")))),
	Ref_Specific=c(length(intersect(which(analysisresults$region=="promoter"),
                which(analysisresults$REaltrecateg=="Reference Specific"))),
                length(intersect(which(analysisresults$region=="enhancer"),
                which(analysisresults$REaltrecateg=="Reference Specific")))),
	Shared=c(length(intersect(which(analysisresults$region=="promoter"),
                which(analysisresults$REaltrecateg=="Shared"))),
                length(intersect(which(analysisresults$region=="enhancer"),
                which(analysisresults$REaltrecateg=="Shared")))),
	Ambiguous=c(length(intersect(which(analysisresults$region=="promoter"),
                which(analysisresults$REaltrecateg=="Ambiguous"))),
                length(intersect(which(analysisresults$region=="enhancer"),
                which(analysisresults$REaltrecateg=="Ambiguous")))))
   print(stats)

   result=list(analysisresults=analysisresults,
	stats=stats)

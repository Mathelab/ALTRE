...
#' }
#' @return ways -- pathways also annotated with additional information run
#' with categaltre_peaks
#' @export
#'
runGREAT <- function(peaks,
  ...
  for (i in c("Experiment Specific", "Reference Specific", "Shared")) {
    ...
    mypeaks <- as.data.frame(peaks$analysisresults)[
      which(peaks$analysisresults$REaltrecateg == i), c("chr", "start", "stop")]
    ilabel <- gsub(" ", "_", i)
    # Run GREAT
...
processPathways <- function(GREATpath,
  ...
  for (job in names(GREATpath)) {
    ...
    output <- rGREAT::getEnrichmentTables(GREATpath[[job]],
                                          category = pathway_category)
    stats <- data.frame(
      Pathway = names(output),
      NumSig_Binom = rep(NA, length(names(output))),

...
getcounts <- function(annotpeaks, csvfile, reference, chrom = NULL) {
    ...
    countsse <- GenomicAlignments::summarizeOverlaps(features = inputgranges, reads = bamfileslist, mode = "Union", singleEnd = TRUE, ignore.strand = TRUE)
    # add column labels
    ...
    # Optional filtering out of lowcount regions As part of the DESeq2 algorithm, more stringent filtering will be applied subsequently countssedds[
    # rowSums(counts(countssedds)) > 1, ]
    ...
    # Calculate RPKM for plotting densities multiply by 10^6 and divide by regions size to get rpkm
    ...
    statdf = data.frame(Num_Enhancers = enhancernum, Num_Promoters = promoternum)
    
    ########################################## Create densityplot
    region = originaldata$region
    forplotdf = cbind(myrpkmlog2, as.data.frame(region))
    
    return(list(regioncounts = countssedds, regioncountstats = statdf, regioncountsforplot = forplotdf))
}

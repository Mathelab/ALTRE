...
getcounts <-
  function(annotpeaks, csvfile, reference, chrom = NULL) {
    ...
    #add column labels
    SummarizedExperiment::colData(countsse) <-
      DataFrame(sampleinfo[, c(1:4)])
    ...
    colnames(originaldata) = unlist(lapply(

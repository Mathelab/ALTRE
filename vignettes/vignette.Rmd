---
title: "RRDESeq2 Vignette"
author: "Elizabeth Baskin, Wolfgang Resch, and Ewy Mathe"
date: "April 7, 2016"
output: pdf_document
---
...
As for all high-throughput sequencing experiments, the results of ATAC-seq, DHS-seq and FAIRE-seq experiments are delivered in FASTQ files. The initial processing of the FASTQ file must be accomplished outside of RRDESeq2, as the R-package requires BAM and hotspot files as inputs. Alignment of sequencing reads produces BAM files, and peak identification produces hotspot files. There are a number of software packages available for both alignment and hotspot calling; we recommend bowtie for alignment, and dnase2hotspot for hotspot calling.

RRDESeq2 builds a number of packages 

#Example data 

To demonstrate the abilities of RRDESeq2, we have provided an example dataset (BAM and HOTSPOT files) of cancerous (A549) and normal (SAEC) lung cell types downloaded from the ENCODE project. This dataset will guide users through the workflow of the package in this vignette. 
...
```{r}
dir=system.file("extdata", package="RRDESeq2", mustWork=TRUE)
dir
```
...
```{r}
allregionsannotated_lung=annotateandmerge(hotspots_lung, TSS=TSSannot)
```
...
```{r}
getresults(allregionsannotated_lung)
```
...
```{r}
getmetrics(allregionsannotated_lung)
```
...
```{r}
counts_lung=getcounts(hotspot=allregionsannotated_lung, csv=csvpath, percentile=0.5, normal="SAEC")
```
...
```{r eval=FALSE}
getresults(counts_lung)
```

```{r echo=FALSE}
head(results(counts_lung))
```
...
```{r}
analysisresults_lung=countanalysis(counts_lung, alphavalue=0.01, lfcvalue=1, normal="SAEC")
```

The results are p-values, log2fold, and other statistics for every region. 

```{r eval=FALSE}
getresults(analysisresults_lung)
```

```{r echo=FALSE}
head(results(analysisresults_lung))
```
...
Pathways that are unlikely to be of interest can be filtered out of the enrichment results. RRDESeq2 has two metrics to identify "low information" pathways.  
...
```{r}
lung_MF=enrichment(TSSannot, analysisresults_lung, ontoltype="MF", pvalfilt=0.01)
```
...
Once results have been obtained, there are a number of additional functions in RRDESeq2 to summarize and better depict the results and raw data in tables and graphs. In this section of the vignette we will walk through these functions.
...
```{r}
resultscomparison(analysisresults_lung, normal="SAEC")
